...
*****************************************************************************************************/
cap log close
cap log using sae18_log_test_matchvars_nn, text replace 
...
local dataout "$analysis\SAE_MATA_wd18"
local dataoutu "$analysis\SAE_MATAu_wd18"
local dataoutr "$analysis\SAE_MATAr_wd18"


local ydump "$analysis\mata_wd18"
local ydumpu "$analysis\matau_wd18"
local ydumpr "$analysis\matar_wd18"
...
/*
foreach var in Dagecat4ht casualurbanht regwageurbanht hhrh hrh recr recrh transpr transprh hhrt hhrht recrt  recrht transprt transprht {
local xvar111t : subinstr local xvar111t "`var’" "" 
}

foreach var in Dagecat4ht casualurbanht regwageurbanht hhrh hrh recr recrh transpr transprh hhrt hhrht recrt  recrht transprt transprht {
local xvar011t : subinstr local xvar011t "`var'" "" 
}
*/
...
estimates save "$estimates/stage1_18_indu_wd", replace 
...
estimates save "$estimates/stage1_18_indr_wd", replace 
...
************************************************* Second Stage - Urban **********************************************
sae data import, varlist( `xvar111t' qhweight qhclust state district_code) uniqid(id_sae) area(district_code) ///
...
pline(57.791668) addvars(id_sae qhclust psu qhnum `xvar111t' state district_code) 
*/ 
...
pline(57.791668) addvars(id_sae qhclust `xvar111t') 
*/ 

* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd18)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd18.dta", replace

************************************************* Second Stage - Rural **********************************************
sae data import, varlist( `xvar011t' qhweight qhclust state district_code) uniqid(id_sae) area(district_code) ///
...
pline(57.791668) addvars(id_sae qhclust psu qhnum `xvar011t' state district_code) 
*/ 
...
pline(57.791668) addvars(id_sae qhclust `xvar011t')  
*/ 

* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd18)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd18.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd18.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd18.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd18.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd18", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd18", clear
	...
	** B = 1/(m-1) Sum (Ghat - Gbar)^2 where Ghat is gini estimate and Gbar is average across imputations 
	...
	qui summ giniu 
	local B=r(sd)^2 
	scalar giniu_18=r(mean) 
	dis 
	dis "Gini estimate is " %5.3f `giniu_18' " In urban areas" 
	scalar gini_18_sdu=(`W'+(1.01)*`B')^0.5 
	dis "Standard deviation is: " %5.3f `gini_18_sd' 
	
	qui summ ginir_var 
	local W=r(mean) 
	qui summ ginir 
	local B=r(sd)^2 
	scalar ginir_18=r(mean) 
	dis "Gini estimate is " %5.3f `ginir_18' " In rural areas" 
	scalar gini_18_sdr=(`W'+(1.01)*`B')^0.5 
	dis "Standard deviation is: " %5.3f `gini_18_sd' 
	
	mat a=J(2,4,.)
	mat a[1,1]=giniu_18*100 
	mat a[2,1]=ginir_18*100
	mat a[1,2]= (giniu_18 - 1.96*(gini_18_sdu))*100
	mat a[1,3]=(giniu_18 + 1.96*(gini_18_sdu))*100
	mat a[2,2]=(ginir_18 - 1.96*(gini_18_sdr))*100
	mat a[2,3]=(ginir_18 + 1.96*(gini_18_sdr))*100
	mat a[1,4]=39.0184
	mat a[2,4]=31.1275
...
posthead(& {\textbf{2018.5}} & \multicolumn{2}{c}{\textbf{95\% C.I}} & {\textbf{2011.5$^a$}} \\ "\hline") replace
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae18_wd", replace 
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae18_completedistrictsubsample_wd", replace 
	restore 
