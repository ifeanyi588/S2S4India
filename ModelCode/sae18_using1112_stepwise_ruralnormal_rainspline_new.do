...
cap log using sae18_log_using1112_stepwise_ruralnormal_rainspline_new, text replace
...
local dataout "$analysis\SAE_MATA18_using1112_stepwise_ruralnormal_rainspline_new"
local dataoutu "$analysis\SAE_MATA18u_using1112_stepwise_ruralnormal_rainspline_new"
local dataoutr "$analysis\SAE_MATA18r_using1112_stepwise_ruralnormal_rainspline_new"


local ydump "$analysis\mata18_using1112_stepwise_ruralnormal_rainspline_new"
local ydumpu "$analysis\mata18u_using1112_stepwise_ruralnormal_rainspline_new"
local ydumpr "$analysis\mata18r_using1112_stepwise_ruralnormal_rainspline_new"
...
estimates save "$estimates/stage1u_18_using1112_stepwise_ruralnormal_rainspline_new", replace 

sae model lmm lnwelf $urbanvars [pw=wgt], area(id_sae_1) varest(h3) lny zvar($zvar) yhat($yhat) yhat2($yhat2) 
predict lnwelfhat, xb
gen urbanerr = lnwelf - lnwelfhat
sktest urbanerr //test for the normality of the error distribution 
su urbanerr, detail //check for skewness (around 0?) and kurtosis (around 3?)
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_18_using1112_stepwise_ruralnormal_rainspline_new)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_18_using1112_stepwise_ruralnormal_rainspline_new.dta", replace
...
estimates save "$estimates/stage1r_18_using1112_stepwise_ruralnormal_rainspline_new", replace 

sae model lmm lnwelf $ruralvars [pw=wgt], area(id_sae_1) varest(h3) lny zvar($zvar) yhat($yhat) yhat2($yhat2) 
predict ruralwelfhat, xb
gen ruralerr = lnwelf - ruralwelfhat
sktest ruralerr  //test for normality of the distribution
su ruralerr, detail // just to see what the specifics are on skewness and kurtosis
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_18_using1112_stepwise_ruralnormal_rainspline_new)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_18_using1112_stepwise_ruralnormal_rainspline_new.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_18_using1112_stepwise_ruralnormal_rainspline_new.dta", clear
	gen urban=1
	append using "$analysis\final_datar_18_using1112_stepwise_ruralnormal_rainspline_new.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_18_using1112_stepwise_ruralnormal_rainspline_new.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_18_using1112_stepwise_ruralnormal_rainspline_new", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_18_using1112_stepwise_ruralnormal_rainspline_new", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_using1112_stepwise_ruralnormal_rainspline_new", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_using1112_stepwise_ruralnormal_rainspline_new", replace
