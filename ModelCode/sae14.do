...
estimates save "$estimates/stage1_14_indu", replace 
...
estimates save "$estimates/stage1_14_indr", replace 

qui: lassoregress robust_alfa `xvar011t' [aw=pop_wgt]

local error_lasso_modelr "`e(varlist_nonzero)'"

estout indu indr using "$tables/indfirsta.tex", ///
...
estout indu indr using "$tables/indfirstb.tex", ///
cells(b(star fmt(%9.2f)) se(par fmt(%9.2f))) stats(r2_beta N_beta F_beta, fmt(%9.2f %9.0f %9.0f) labels("\$R^2\$" "N" "F-Stat" )) collabels(none) style(tex) starlevels(* .10 ** .05 *** .01) mlabels(none) ///
...
estout  indu indr using "$tables/indfirstc.tex", ///
cells(b(star fmt(%9.2f)) se(par fmt(%9.2f))) stats(r2_beta N_beta F_beta, fmt(%9.2f %9.0f %9.0f) labels("\$R^2\$" "N" "F-Stat" )) collabels(none) style(tex) starlevels(* .10 ** .05 *** .01) mlabels(none) ///
...
estout  indu indr using "$tables/indfirstd.tex", ///
cells(b(star fmt(%9.2f)) se(par fmt(%9.2f))) stats(r2_beta N_beta F_beta, fmt(%9.2f %9.0f %9.0f) labels("\$R^2\$" "N" "F-Stat" )) collabels(none) style(tex) starlevels(* .10 ** .05 *** .01) mlabels(none) ///
...
************************************************* Second Stage - Urban **********************************************
sae data import, varlist( `xvar111t' qhweight qhclust psu qhnum state district Nyearsdistrict) uniqid(id_sae) area(id_sae_1) ///
...
pline(57.791668) addvars(id_sae qhclust psu qhnum `xvar111t' state district Nyearsdistrict) 
*/ 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu.dta", replace

************************************************* Second Stage - Rural **********************************************
sae data import, varlist( `xvar011t' qhweight qhclust psu qhnum state district Nyearsdistrict) uniqid(id_sae) area(id_sae_1) ///
...
pline(57.791668) addvars(id_sae qhclust psu qhnum `xvar011t' state district Nyearsdistrict) 
*/ 
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau.dta", clear
	gen urban=1
	append using "$analysis\final_datar.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur.dta", replace
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae14", replace 
	
	** replicate subsample of compete distrits for comparison with 
	preserve 
	keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae14_completedistrictsubsample", replace 
	restore 
