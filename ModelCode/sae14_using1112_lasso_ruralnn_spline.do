...
cap log using sae14_log_using1112_lasso_ruralnn_spline, text replace
...
local dataout "$analysis\SAE_MATA14_using1112_lasso_ruralnn_spline"
local dataoutu "$analysis\SAE_MATA14u_using1112_lasso_ruralnn_spline"
local dataoutr "$analysis\SAE_MATA14r_using1112_lasso_ruralnn_spline"


local ydump "$analysis\mata14_using1112_lasso_ruralnn_spline"
local ydumpu "$analysis\mata14u_using1112_lasso_ruralnn_spline"
local ydumpr "$analysis\mata14r_using1112_lasso_ruralnn_spline"
...
*estimates save "$estimates/stage1u_14_using1112_lasso_ruralnn_spline", replace 

//quickly generate errors to select variables for the alpha model
*sae model povmap lnwelf $urbanvars2 [pw=pop_wgt], area(district_code) varest(h3) alfatest(_res) zvar($urbanvars2)
...
gen robust_alfa = lnwelf - lnwelfhat
su robust_alfa, detail
sktest robust_alfa //test the distribution of the errors for normality (skewness should be 0, kurtosis should be 3 for normality)

*estimates store indr

*estimates save "$estimates/stage1r_14_using1112_lasso_ruralnn_spline", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_14_using1112_lasso_ruralnn_spline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_14_using1112_lasso_ruralnn_spline.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_14_using1112_lasso_ruralnn_spline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_14_using1112_lasso_ruralnn_spline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_14_using1112_lasso_ruralnn_spline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_14_using1112_lasso_ruralnn_spline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_14_using1112_lasso_ruralnn_spline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_14_using1112_lasso_ruralnn_spline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_14_using1112_lasso_ruralnn_spline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae14_sectorpov_using1112_lasso_ruralnn_spline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae14_overallpov_using1112_lasso_ruralnn_spline", replace
