...
local dataout "$analysis\SAE_MATA_wd18_newvars_withselection"
local dataoutu "$analysis\SAE_MATAu_wd18_newvars_withselection"
local dataoutr "$analysis\SAE_MATAr_wd18_newvars_withselection"


local ydump "$analysis\mata_wd18_newvars_withselection"
local ydumpu "$analysis\matau_wd18_newvars_withselection"
local ydumpr "$analysis\matar_wd18_newvars_withselection"
...
estimates save "$estimates/stage1_18_indu_newvars_withselection", replace 
...
estimates save "$estimates/stage1_18_indr_newvars_withselection", replace 
...
pline(57.791668) addvars(id_sae qhclust psu qhnum $urbanvars2) 
*/ 

* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd18_newvars_withselection)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd18_newvars_withselection.dta", replace
...
pline(57.791668) addvars(id_sae qhclust psu qhnum $ruralvars)  
*/ 

* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd18_newvars_withselection)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd18_newvars_withselection.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd18_newvars_withselection.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd18_newvars_withselection.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd18_newvars_withselection.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd18_newvars_withselection.dta", replace 
	
	mi passive: gen Dpoor190=(welfare<57.791668)
	
	mi passive: gen Dpoor320=(welfare<57.791668*(3.2/1.9))	
	
	mi passive: gen Dpoor550=(welfare<57.791668*(5.5/1.9))	
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd18_newvars_withselection", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae18_newvars_withselection", replace 
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae18_completedistrictsubsample_newvars_withselection", replace 
	restore 
	*/
