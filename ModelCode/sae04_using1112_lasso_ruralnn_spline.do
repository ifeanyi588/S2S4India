...
cap log using sae04_log_using1112_lasso_ruralnn_spline, text replace
...
local dataout "$analysis\SAE_MATA04_using1112_lasso_ruralnn_spline"
local dataoutu "$analysis\SAE_MATA04u_using1112_lasso_ruralnn_spline"
local dataoutr "$analysis\SAE_MATA04r_using1112_lasso_ruralnn_spline"


local ydump "$analysis\mata04_using1112_lasso_ruralnn_spline"
local ydumpu "$analysis\mata04u_using1112_lasso_ruralnn_spline"
local ydumpr "$analysis\mata04r_using1112_lasso_ruralnn_spline"
...
*estimates save "$estimates/stage1u_04_using1112_lasso_ruralnn_spline", replace 
...
*estimates save "$estimates/stage1r_04_using1112_lasso_ruralnn_spline", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_04_using1112_lasso_ruralnn_spline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_04_using1112_lasso_ruralnn_spline.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_04_using1112_lasso_ruralnn_spline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_04_using1112_lasso_ruralnn_spline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_04_using1112_lasso_ruralnn_spline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_04_using1112_lasso_ruralnn_spline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_04_using1112_lasso_ruralnn_spline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_04_using1112_lasso_ruralnn_spline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_04_using1112_lasso_ruralnn_spline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae04_sectorpov_using1112_lasso_ruralnn_spline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae04_overallpov_using1112_lasso_ruralnn_spline", replace
