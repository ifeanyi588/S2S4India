...
cap log using sae18_log_using1112_stepwise_allnormal_rainspline_v3, text replace


cap adopath - "C:\Users\wb559885\Desktop\SAE-Stata-Package-master\s"
* adopath ++ "C:\Users\wb559885\Desktop\SAE-Stata-Package-master\s"
adopath ++ "D:\Poverty_Estimates_India\SAE_India\sae_v3\SAE-Stata-Package-master"
...
local dataout "$analysis\SAE_MATA18_using1112_stepwise_allnormal_rainspline_v3"
local dataoutu "$analysis\SAE_MATA18u_using1112_stepwise_allnormal_rainspline_v3"
local dataoutr "$analysis\SAE_MATA18r_using1112_stepwise_allnormal_rainspline_v3"


local ydump "$analysis\mata18_using1112_stepwise_allnormal_rainspline_v3"
local ydumpu "$analysis\mata18u_using1112_stepwise_allnormal_rainspline_v3"
local ydumpr "$analysis\mata18r_using1112_stepwise_allnormal_rainspline_v3"
...
stepwise, pr(0.05): regress lnwelf $urbanvars [pw = wgt]
local temp : colfullnames e(b) 
local b _cons
local xvars : list temp - b
...
estimates save "$estimates/stage1u_18_using1112_stepwise_allnormal_rainspline_v3", replace 
...
* non-parametric
/*
sae sim h3 lnwelf $urbanvars [aw=pop_wgt], area(district_code) uniqid(id_sae) mcrep(100) bsrep(100) seed(57678) /*bootstrap*/ lny  zvar($zvar) yhat($yhat) yhat2($yhat2) aggids(7) ydump(`ydumpu') pwcensus(qhweight) matin(`dataoutu') indicators(FGT0) /*allmata zvar(`error_lasso_modelu')*/ ///
pline(57.791668) addvars(id_sae qhweight qhclust $urbanvars) 
*/ 
set trace on
set traced 2 
sae sim h3 lnwelf $urbanvars [aw= pop_wgt], area(district_code) uniqid(id_sae) mcrep(100) bsrep(100) ///
                             matin(`dataoutu') zvar($zvar) yhat($yhat) yhat2($yhat2) ///
                             pwcensus(qhweight) aggids(7) ///
                             ind(FGT0) plines(57.791668) ydump(`ydumpu') lny addvars(id_sae qhweight qhclust $urbanvars)


* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_18_using1112_stepwise_allnormal_rainspline_v3)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_18_using1112_stepwise_allnormal_rainspline_v3.dta", replace
...
estimates save "$estimates/stage1r_18_using1112_stepwise_allnormal_rainspline_v3", replace 
...
sae sim h3 lnwelf $ruralvars [aw=pop_wgt], area(district_code) varest(h3) uniqid(id_sae) mcrep(100) bsrep(100)  seed(16757) lny zvar($zvar) yhat($yhat) yhat2($yhat2) aggids(7) ydump(`ydumpr') pwcensus(qhweight) matin(`dataoutr') indicators(FGT0) /*allmata zvar(`error_lasso_modelr')*/ ///
pline(57.791668) addvars(id_sae qhweight qhclust $ruralvars district_code) 
*/ 

sae sim h3 lnwelf $ruralvars [aw= pop_wgt], area(district_code) uniqid(id_sae) mcrep(100) bsrep(100) ///
                             matin(`dataoutu') zvar($zvar) yhat($yhat) yhat2($yhat2) ///
                             pwcensus(qhweight) aggids(7 8)///
                             ind(FGT0) plines(57.791668) ydump(`ydumpu') lny addvars(id_sae qhweight qhclust $ruralvars)
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_18_using1112_stepwise_allnormal_rainspline_v3)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_18_using1112_stepwise_allnormal_rainspline_v3.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_18_using1112_stepwise_allnormal_rainspline_v3.dta", clear
	gen urban=1
	append using "$analysis\final_datar_18_using1112_stepwise_allnormal_rainspline_v3.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_18_using1112_stepwise_allnormal_rainspline_v3.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_18_using1112_stepwise_allnormal_rainspline_v3", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_18_using1112_stepwise_allnormal_rainspline_v3", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_using1112_stepwise_allnormal_rainspline_v3", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_using1112_stepwise_allnormal_rainspline_v3", replace
