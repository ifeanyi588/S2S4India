...
cap log using sae09_log_using1112_allvars_nonnormal, text replace
...
local dataout "$analysis\SAE_MATA09_using1112_allvars_nonnormal"
local dataoutu "$analysis\SAE_MATA09u_using1112_allvars_nonnormal"
local dataoutr "$analysis\SAE_MATA09r_using1112_allvars_nonnormal"


local ydump "$analysis\mata09_using1112_allvars_nonnormal"
local ydumpu "$analysis\mata09u_using1112_allvars_nonnormal"
local ydumpr "$analysis\mata09r_using1112_allvars_nonnormal"
...
estimates save "$estimates/stage1_09_indu_using1112_allvars_nonnormal", replace 
...
estimates save "$estimates/stage1_09_indr_using1112_allvars_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_data09u_using1112_allvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu09_using1112_allvars_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_data09r_using1112_allvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr09_using1112_allvars_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_data09u_using1112_allvars_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_data09r_using1112_allvars_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_data09ur_using1112_allvars_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data09_using1112_allvars_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data09_using1112_allvars_nonnormal", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae09_sectorpov_using1112_allvars_nonnormal", replace 
	
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae09_overallpov_using1112_allvars_nonnormal", replace
