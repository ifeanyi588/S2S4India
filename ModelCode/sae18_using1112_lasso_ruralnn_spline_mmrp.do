...
cap log using sae18_log_using1112_lasso_ruralnn_spline_mmrp, text replace
...
local dataout "$analysis\SAE_MATA18_using1112_lasso_ruralnn_spline_mmrp"
local dataoutu "$analysis\SAE_MATA18u_using1112_lasso_ruralnn_spline_mmrp"
local dataoutr "$analysis\SAE_MATA18r_using1112_lasso_ruralnn_spline_mmrp"


local ydump "$analysis\mata18_using1112_lasso_ruralnn_spline_mmrp"
local ydumpu "$analysis\mata18u_using1112_lasso_ruralnn_spline_mmrp"
local ydumpr "$analysis\mata18r_using1112_lasso_ruralnn_spline_mmrp"
...
* First Stage
use "$path\data\finaldata\newmaster_mmrp.dta", clear

gen lnwelf = ln(mpce_mmrp)
...
qui: lassoregress lnwelf $urbanvars [aw=pwt] if urban == 1
glo urbanvars2 "`e(varlist_nonzero)'"

qui: lassoregress lnwelf $ruralvars [aw=pwt] if urban == 0
...
regress lnwelf $urbanvars2 [aw=pwt], vce(robust) 

sae model povmap lnwelf $urbanvars2 [pw=pwt], area(district_code) varest(h3) alfatest(_res) zvar($urbanvars2)
...
*estimates save "$estimates/stage1u_18_using1112_lasso_ruralnn_spline_mmrp", replace 

//quickly generate errors to select variables for the alpha model
*sae model povmap lnwelf $urbanvars2 [pw=pwt], area(district_code) varest(h3) alfatest(_res) zvar($urbanvars2)

qui: lassoregress robust_alfa $urbanvars2 [aw=pwt]
...
sae model povmap lnwelf $ruralvars2 [pw=pwt], area(district_code) varest(h3) alfatest(_res) zvar($ruralvars2)
predict lnwelfhat, xb
gen robust_alfa = lnwelf - lnwelfhat
su robust_alfa, detail
...
*estimates save "$estimates/stage1r_18_using1112_lasso_ruralnn_spline_mmrp", replace 


qui: lassoregress robust_alfa $ruralvars [aw=pwt]
...
/* assume normality 
sae sim povmap lnwelf $urbanvars2 [pw=pwt], area(district_code) varest(ell) eta(normal) epsilon(normal) ///
...
* non-paramertric
sae sim povmap lnwelf $urbanvars2 [pw=pwt], area(district_code) varest(h3) eta(nonnormal) epsilon(nonnormal) ///
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_18_using1112_lasso_ruralnn_spline_mmrp)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_18_using1112_lasso_ruralnn_spline_mmrp.dta", replace
...
keep if urban==0
* assume normality  
sae sim povmap lnwelf $ruralvars2 [pw=pwt], area(district_code) varest(h3) eta(nonnormal) epsilon(nonnormal) ///
...
sae sim povmap lnwelf $ruralvars2 [pw=pwt], area(district_code) varest(h3) eta(normal) epsilon(normal) ///
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_18_using1112_lasso_ruralnn_spline_mmrp)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_18_using1112_lasso_ruralnn_spline_mmrp.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_18_using1112_lasso_ruralnn_spline_mmrp.dta", clear
	gen urban=1
	append using "$analysis\final_datar_18_using1112_lasso_ruralnn_spline_mmrp.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_18_using1112_lasso_ruralnn_spline_mmrp.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_18_using1112_lasso_ruralnn_spline_mmrp", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_18_using1112_lasso_ruralnn_spline_mmrp", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_using1112_lasso_ruralnn_spline_mmrp", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_using1112_lasso_ruralnn_spline_mmrp", replace
...
use  "$data\finaldata\hh_reg.dta", clear
	...
	collapse (mean) Dpoor190 [pw=pwt], by(state)
...
mean poor_int poor_320 poor_550 [pw=pwt], over (urban year)
mat x=r(table)
...
mean poor_int [pw=pwt], over (urban)
mat x04=r(table)'
mat list x04
