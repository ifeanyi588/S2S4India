...
******************************************************************************************************
**                                                                                                  **
**                  IMPUTATION OF CONSUMPTION EXPD IN THE 2009 DATA                                  **
**                                                                                                  **
** COUNTRY			INDIA
...
*****************************************************************************************************
*                                                                                                   *                          
***** --- ELL ESTIMATION: Uses the 2004.5, 2009.5 and 2011.5 surveys to impute into 2009-15 --- *****
*                                                                                                   *
*****************************************************************************************************/
...
cap log close
*cap log using sae09_log_test_matchvars, text replace 
*cap log using sae09_log_test_matchvars_notime, text replace
*cap log using sae09_log_test_newvars, text replace
*cap log using sae09_log_test_newvars_withselection, text replace
cap log using sae09_log_using1112_sw_ruralnn_rainsq, text replace


* locals for directories and variables

local datain "$data2\sae09.dta"
local datainu "$data2\sae09u.dta"
local datainr "$data2\sae09r.dta"

local dataout "$analysis\SAE_MATA09_using1112_sw_ruralnn_rainsq"
local dataoutu "$analysis\SAE_MATA09u_using1112_sw_ruralnn_rainsq"
local dataoutr "$analysis\SAE_MATA09r_using1112_sw_ruralnn_rainsq"


local ydump "$analysis\mata09_using1112_sw_ruralnn_rainsq"
local ydumpu "$analysis\mata09u_using1112_sw_ruralnn_rainsq"
local ydumpr "$analysis\mata09r_using1112_sw_ruralnn_rainsq"
...
sae model lmm lnwelf $urbanvars [pw=wgt], area(district_code)  varest(h3)
...
global zvar hhsizeh
sae model lmm lnwelf $urbanvars [pw=wgt], area(district_code) lny varest(h3)  alfatest(_res) zvar($zvar) 
...
sae model lmm lnwelf $urbanvars [pw=wgt], area(district_code) varest(h3) lny zvar($zvar) yhat($yhat) yhat2($yhat2) 
predict lnwelfhat, xb
gen robust_alfa = lnwelf - lnwelfhat
su robust_alfa, detail
sktest robust_alfa

*estimates table 

*estimates store indu
*estimates save "$estimates/stage1u_09_using1112_sw_ruralnn_rainsq", replace 
...
sae model lmm lnwelf $ruralvars [pw=wgt], area(district_code)  varest(h3)
...
* Getting the dependent variable for the alpha model
global zvar hhsizeh
sae model lmm lnwelf $ruralvars [pw=wgt], area(district_code) lny varest(h3) alfatest(_res) zvar($zvar) 
...
* then you pass that to the final model and see the GLS estimates
sae model lmm lnwelf $ruralvars [pw=wgt], area(district_code) varest(h3) lny zvar($zvar) yhat($yhat) yhat2($yhat2) 
predict lnwelfhat, xb
gen robust_alfa = lnwelf - lnwelfhat
su robust_alfa, detail
sktest robust_alfa
...
*estimates save "$estimates/stage1r_09_using1112_sw_ruralnn_rainsq", replace 
...
************************************************* Second Stage - Urban **********************************************
sae data import, varlist(id_sae qhweight qhclust $urbanvars $zvar district_code) uniqid(id_sae) area(district_code) ///
...
uniqid(id_sae) vce(ell) rep(100) seed(57678) lny aggids(7) col(1) stage(second) ydump(`ydumpu') zvar($zvar) yhat($yhat) yhat2($yhat2) ///
res(`resr') pwcensus(qhweight) psu(qhclust) matin(`dataoutu') indicators(fgt0) /*allmata zvar(`error_lasso_modelu')*/ ///
pline(57.791668) addvars(id_sae qhweight qhclust $urbanvars $zvar) 
*/ 

* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_09_using1112_sw_ruralnn_rainsq)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_09_using1112_sw_ruralnn_rainsq.dta", replace

************************************************* Second Stage - Rural **********************************************
sae data import, varlist(id_sae qhweight qhclust $ruralvars $zvar district_code) uniqid(id_sae) area(district_code) ///
...
uniqid(id_sae) vce(ell) rep(100) seed(16757) lny aggids(7) col(1) stage(second) ydump(`ydumpr') zvar($zvar) yhat($yhat) yhat2($yhat2) ///
res(`resr') pwcensus(qhweight) psu(qhclust) matin(`dataoutr') indicators(fgt0 gini) /*allmata zvar(`error_lasso_modelr')*/ ///
pline(57.791668) addvars(id_sae qhweight qhclust $ruralvars $zvar district_code) 
*/ 
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_09_using1112_sw_ruralnn_rainsq)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_09_using1112_sw_ruralnn_rainsq.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_09_using1112_sw_ruralnn_rainsq.dta", clear
	gen urban=1
	append using "$analysis\final_datar_09_using1112_sw_ruralnn_rainsq.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_09_using1112_sw_ruralnn_rainsq.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_09_using1112_sw_ruralnn_rainsq", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_09_using1112_sw_ruralnn_rainsq", clear
	
	
	
	scalar ppp_rur = 12.90806
	scalar ppp_urb = 15.69503
	scalar newppp_rur=13.1734532630737  
	scalar newppp_urb=16.0177240707475  
	
	mi passive: gen welfare_newppp=welfare*1/(newppp_urb/ppp_urb) if urban==1 
	mi passive: replace welfare_newppp=welfare*1/(newppp_rur/ppp_rur) if urban==0 
	...
	dis "`suffix'" 
	qui mi passive: gen Dpoor`suffix'=(welfare_newppp<57.791668*`line'/1.9)
	qui mi passive: gen povgap`suffix'=max(1-welfare_newppp/(57.791668*`line'/1.9),0) 
	qui mi passive: gen povseverity`suffix'=max(1-welfare_newppp/(57.791668*`line'/1.9),0)^2 
	}
	...
	save "$data/state_povrates_190_2009", replace 
	restore 
	*/ 
	mi describe
	...
	estimates save "$pathout\estimates\sae09_sectorpov_using1112_sw_ruralnn_rainsq", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae09_overallpov_using1112_sw_ruralnn_rainsq", replace
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae09_completedistrictsubsample_newvars_withselection_nonnormal", replace 
	restore 
	...
	mat p09=J(2,3,.)
	mat p09[1,1]=est_poverty[1,2]*100 // % poverty headcount
	mat p09[1,2]=est_poverty[5,2]*100 // % poverty headcount
	mat p09[1,3]=est_poverty[6,2]*100 // % poverty headcount
	
	mat p09[2,1]=est_poverty[1,1]*100
	mat p09[2,2]=est_poverty[5,1]*100
	mat p09[2,3]=est_poverty[6,1]*100

	mat p09_320=J(2,3,.) 
	mat p09_320[1,1]=est_poverty[1,4]*100 // % poverty headcount
	mat p09_320[1,2]=est_poverty[5,4]*100 // % poverty headcount
	mat p09_320[1,3]=est_poverty[6,4]*100 // % poverty headcount
	
	mat p09_320[2,1]=est_poverty[1,3]*100
	mat p09_320[2,2]=est_poverty[5,3]*100
	mat p09_320[2,3]=est_poverty[6,3]*100
	
	mat p09_550=J(2,3,.) 
	mat p09_550[1,1]=est_poverty[1,6]*100 // % poverty headcount
	mat p09_550[1,2]=est_poverty[5,6]*100 // % poverty headcount
	mat p09_550[1,3]=est_poverty[6,6]*100 // % poverty headcount
	
	mat p09_550[2,1]=est_poverty[1,5]*100
	mat p09_550[2,2]=est_poverty[5,5]*100
	mat p09_550[2,3]=est_poverty[6,5]*100
	
	
	
	
svmat p09, names(poor09)
svmat p09_320, names(poor09_320) 
svmat p09_550, names(poor09_550) 		

keep poor09*
drop if poor091==. 
save "$analysis\pov09sae.dta", replace
...
use  "$data\finaldata\hh_reg.dta", clear
	...
	forvalues y=1/2{
	foreach i in 1 5 6{
	mat r09`y'`i'=r09[`y',`i']*100
	}
	mat z`y'= r09`y'1, r09`y'5, r09`y'6
	}
...
mat rownames k09= "\textbf{Urban}" "\textbf{Rural}"
esttab matrix (k09, fmt(%9.1fc)) using "$tables/sae09.tex", /// 
nonumbers nomtitles nodepvars align(rrrrrr) style(tab) ///
collabel(none) modelwidth(10) ///
addnotes("Sources: India National Sample Survey Office (NSSO) Surveys." "Using 2004, 2009 and 2011 Surveys to project into 2009." "a. Simulated" "b. Actual") ///
posthead(& \multicolumn{1}{c}{\color{red}\textbf{2009.5$^a$}} & \multicolumn{2}{c}{\textbf{95\% C.I}} & \multicolumn{1}{c}{\textbf{2011.5$^b$}} & \multicolumn{2}{c}{\textbf{95\% C.I}}\\ "\hline") replace type

** 3.20 line 	
	gen poor_320=welfare*(12/365)/(cpi*ppp)<3.20
	svy: mean poor_320, over(urban) 
	mat r09_320=r(table)' 
	matrix z09_320=r09_320[1..2,1],r09_320[1..2,5..6] 
	matrix z09_320=z09_320*100 
	matrix k09_320=p09_320, z09_320
	mat rownames k09_320= "\textbf{Urban}" "\textbf{Rural}" 
esttab matrix (k09_320, fmt(%9.1fc)) using "$tables/sae09_320.tex", /// 
nonumbers nomtitles nodepvars align(rrrrrr) style(tab) ///
collabel(none) modelwidth(10) ///
addnotes("Sources: India National Sample Survey Office (NSSO) Surveys." "Using 2004, 2009 and 2011 Surveys to project into 2009." "a. Simulated" "b. Actual") ///
posthead(& \multicolumn{1}{c}{\color{red}\textbf{2009.5$^a$}} & \multicolumn{2}{c}{\textbf{95\% C.I}} & \multicolumn{1}{c}{\textbf{2011.5$^b$}} & \multicolumn{2}{c}{\textbf{95\% C.I}}\\ "\hline") replace type
...
** 5.50 line 	
	...
	mat r09_550=r(table)' 
	matrix z09_550=r09_550[1..2,1],r09_550[1..2,5..6] 
	matrix z09_550=z09_550*100 
	matrix k09_550=p09_550, z09_550
	mat rownames k09_550= "\textbf{Urban}" "\textbf{Rural}" 
esttab matrix (k09_550, fmt(%9.1fc)) using "$tables/sae09_550.tex", /// 
nonumbers nomtitles nodepvars align(rrrrrr) style(tab) ///
collabel(none) modelwidth(10) ///
addnotes("Sources: India National Sample Survey Office (NSSO) Surveys." "Using 2004, 2009 and 2011 Surveys to project into 2009." "a. Simulated" "b. Actual") ///
posthead(& \multicolumn{1}{c}{\color{red}\textbf{2009.5$^a$}} & \multicolumn{2}{c}{\textbf{95\% C.I}} & \multicolumn{1}{c}{\textbf{2011.5$^b$}} & \multicolumn{2}{c}{\textbf{95\% C.I}}\\ "\hline") replace type
...
matrix n2=(p09[1,1]*(.316309)+p09[2,1]*(1-.316309))\p09[1..2,1]
matrix  n=n,n2
matrix list n 
matrix drop n2 
...
matrix n2=(p09_320[1,1]*(.316309)+p09_320[2,1]*(1-.316309))\p09_320[1..2,1]
...
** 5.50 line 
matrix n_550=x[1,13..15]*(1-.316309)+x[1,09..16]*(.316309)
matrix n_550=n_550\x[1,09..16]
matrix n_550=n_550\x[1,13..15]
matrix n_550=n_550*100 

matrix n2=(p09_550[1,1]*(.316309)+p09_550[2,1]*(1-.316309))\p09_550[1..2,1]
...
mat n[2,3]=(k09[1,4])
mat n[3,3]=(k09[2,4])
mat n[1,3]=(k09[1,4]*.316309 + k09[2,4]*(1-.316309))

mat n[2,4]=(k09[1,1])
mat n[3,4]=(k09[2,1])
mat n[1,4]=(k09[1,1]*.316309 + k09[2,1]*(1-.316309))
*/ 
...
posthead(& \multicolumn{1}{c}{\textbf{2004.5}} & {\textbf{2009.5}} & {\textbf{2011.5}} & {\color{red}\textbf{2009.5}}\\ "\hline") replace
...
gen labpos=6 
replace labpos=8 if year==2009.5 
...
twoway  (line pov_320National year, lpattern(solid) lcolor(black) lwidth(thick)) (line pov_320Urban year, lpattern(dash) lcolor(purple) lwidth(thick)) (line pov_320Rural year, lpattern(longdash) lcolor(forest_green) lwidth(thick)) ///
		...
		label(3 "Rural") label(4 "Year of Survey")) xmticks(##2) xlabel(2004.5 (5) 2009.5) yscale(r(35 85))  ylabel(30(10)90, format(%2.0f)) ytitle(Percent) graphregion(color(white)) ///
...
twoway  (line pov_550National year, lpattern(solid) lcolor(black) lwidth(thick)) (line pov_550Urban year, lpattern(dash) lcolor(purple) lwidth(thick)) (line pov_550Rural year, lpattern(longdash) lcolor(forest_green) lwidth(thick)) ///
		...
		label(3 "Rural") label(4 "Year of Survey")) xmticks(##2) xlabel(2004.5 (5) 2009.5) yscale(r(70 100)) ylabel(70(5)100, format(%2.0f)) ytitle(Percent) graphregion(color(white)) ///
		note("Sources: India National Sample Survey (NSSO) Surveys.")
