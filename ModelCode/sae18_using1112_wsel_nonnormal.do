...
cap log using sae18_log_using1112_wsel_nonnormal, text replace
...
local dataout "$analysis\SAE_MATA18_using1112_wsel_nonnormal"
local dataoutu "$analysis\SAE_MATA18u_using1112_wsel_nonnormal"
local dataoutr "$analysis\SAE_MATA18r_using1112_wsel_nonnormal"


local ydump "$analysis\mata18_using1112_wsel_nonnormal"
local ydumpu "$analysis\mata18u_using1112_wsel_nonnormal"
local ydumpr "$analysis\mata18r_using1112_wsel_nonnormal"
...
sae model povmap lnwelf $urbanvars2 [pw=pop_wgt], area(district_code) varest(ell) alfatest(_res) zvar($urbanvars2)
estimates table 

estimates store indu
estimates save "$estimates/stage1u_18_using1112_wsel_nonnormal", replace 
...
use "$data2\sae12.dta", clear
keep if urban==0
sae model povmap lnwelf $ruralvars2 [pw=pop_wgt], area(district_code) varest(ell) alfatest(_res) zvar($ruralvars2)
estimates store indr

estimates save "$estimates/stage1r_18_using1112_wsel_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_18_using1112_wsel_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_18_using1112_wsel_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_18_using1112_wsel_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_18_using1112_wsel_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_18_using1112_wsel_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_datar_18_using1112_wsel_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_18_using1112_wsel_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_18_using1112_wsel_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_18_using1112_wsel_nonnormal", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_using1112_wsel_nonnormal", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_using1112_wsel_nonnormal", replace
