...
cap log using sae09_log_using1112_lasso_ruralnn_rainsq, text replace
...
local dataout "$analysis\SAE_MATA09_using1112_lasso_ruralnn_rainsq"
local dataoutu "$analysis\SAE_MATA09u_using1112_lasso_ruralnn_rainsq"
local dataoutr "$analysis\SAE_MATA09r_using1112_lasso_ruralnn_rainsq"


local ydump "$analysis\mata09_using1112_lasso_ruralnn_rainsq"
local ydumpu "$analysis\mata09u_using1112_lasso_ruralnn_rainsq"
local ydumpr "$analysis\mata09r_using1112_lasso_ruralnn_rainsq"
...
*estimates save "$estimates/stage1u_09_using1112_lasso_ruralnn_rainsq", replace 
...
*estimates save "$estimates/stage1r_09_using1112_lasso_ruralnn_rainsq", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_09_using1112_lasso_ruralnn_rainsq)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_09_using1112_lasso_ruralnn_rainsq.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_09_using1112_lasso_ruralnn_rainsq)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_09_using1112_lasso_ruralnn_rainsq.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_09_using1112_lasso_ruralnn_rainsq.dta", clear
	gen urban=1
	append using "$analysis\final_datar_09_using1112_lasso_ruralnn_rainsq.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_09_using1112_lasso_ruralnn_rainsq.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_09_using1112_lasso_ruralnn_rainsq", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_09_using1112_lasso_ruralnn_rainsq", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae09_sectorpov_using1112_lasso_ruralnn_rainsq", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae09_overallpov_using1112_lasso_ruralnn_rainsq", replace
