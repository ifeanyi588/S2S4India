...
*****************************************************************************************************
*                                                                                                   *                          
***** --- ELL ESTIMATION: Uses the 2004.5, 2014.5 and 2011.5 surveys to impute into 2014-15 --- *****
*                                                                                                   *
*****************************************************************************************************/
...
cap log using sae14_log_using1112_sw_ruralnn_rainsq, text replace
...
local dataout "$analysis\SAE_MATA14_using1112_sw_ruralnn_rainsq"
local dataoutu "$analysis\SAE_MATA14u_using1112_sw_ruralnn_rainsq"
local dataoutr "$analysis\SAE_MATA14r_using1112_sw_ruralnn_rainsq"


local ydump "$analysis\mata14_using1112_sw_ruralnn_rainsq"
local ydumpu "$analysis\mata14u_using1112_sw_ruralnn_rainsq"
local ydumpr "$analysis\mata14r_using1112_sw_ruralnn_rainsq"
...
*estimates save "$estimates/stage1u_14_using1112_sw_ruralnn_rainsq", replace 
...
*estimates save "$estimates/stage1r_14_using1112_sw_ruralnn_rainsq", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_14_using1112_sw_ruralnn_rainsq)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_14_using1112_sw_ruralnn_rainsq.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_14_using1112_sw_ruralnn_rainsq)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_14_using1112_sw_ruralnn_rainsq.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_14_using1112_sw_ruralnn_rainsq.dta", clear
	gen urban=1
	append using "$analysis\final_datar_14_using1112_sw_ruralnn_rainsq.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_14_using1112_sw_ruralnn_rainsq.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_14_using1112_sw_ruralnn_rainsq", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_14_using1112_sw_ruralnn_rainsq", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae14_sectorpov_using1112_sw_ruralnn_rainsq", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae14_overallpov_using1112_sw_ruralnn_rainsq", replace
...
addnotes("Sources: India National Sample Survey Office (NSSO) Surveys." "Using 2004, 2014 and 2011 Surveys to project into 2014." "a. Simulated" "b. Actual") ///
...
mat n[1,1] = (x04[2,1]*.316314 + x04[1,1]*(1-.316314))*100	
...
posthead(& \multicolumn{1}{c}{\textbf{2004.5}} & {\textbf{2014.5}} & {\textbf{2011.5}} & {\color{red}\textbf{2014.5}}\\ "\hline") replace
