...
*****************************************************************************************************
*                                                                                                   *                          
***** --- ELL ESTIMATION: Uses the 2014.5, 2014.5 and 2011.5 surveys to impute into 2014-15 --- *****
*                                                                                                   *
*****************************************************************************************************/
...
cap log close
*cap log using sae14_log_test_matchvars, text replace 
*cap log using sae14_log_test_matchvars_notime, text replace
*cap log using sae14_log_test_newvars, text replace
*cap log using sae14_log_test_newvars_withselection, text replace
cap log using sae14_log_using1112_stepwise_ruralnn_rainspline, text replace


* locals for directories and variables

local datain "$data2\scssae14.dta"
local datainu "$data2\scssae14u.dta"
local datainr "$data2\scssae14r.dta"

local dataout "$analysis\SAE_MATA14_using1112_stepwise_ruralnn_rainspline"
local dataoutu "$analysis\SAE_MATA14u_using1112_stepwise_ruralnn_rainspline"
local dataoutr "$analysis\SAE_MATA14r_using1112_stepwise_ruralnn_rainspline"


local ydump "$analysis\mata14_using1112_stepwise_ruralnn_rainspline"
local ydumpu "$analysis\mata14u_using1112_stepwise_ruralnn_rainspline"
local ydumpr "$analysis\mata14r_using1112_stepwise_ruralnn_rainspline"
...
global ruralvars "${ruralvars} DrainfallQ4k1 DrainfallQ4k2 DrainfallQ4k3 rainfallQ4"
...
global urbanvars "${urbanvars} DrainfallQ4k1 DrainfallQ4k2 DrainfallQ4k3 rainfallQ4"
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_14_using1112_stepwise_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_14_using1112_stepwise_ruralnn_rainspline.dta", replace
...
estimates save "$estimates/stage1r_14_using1112_stepwise_ruralnn_rainspline", replace 

sae model lmm lnwelf $ruralvars [pw=wgt], area(district_code) varest(h3) lny zvar($zvar) yhat($yhat) yhat2($yhat2) 
predict lnwelfhat, xb
gen ruralerr = lnwelfhat - lnwelf 
su ruralerr, detail 
sktest ruralerr
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_14_using1112_stepwise_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_14_using1112_stepwise_ruralnn_rainspline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_14_using1112_stepwise_ruralnn_rainspline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_14_using1112_stepwise_ruralnn_rainspline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_14_using1112_stepwise_ruralnn_rainspline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_14_using1112_stepwise_ruralnn_rainspline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_14_using1112_stepwise_ruralnn_rainspline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae14_sectorpov_using1112_stepwise_ruralnn_rainspline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae14_overallpov_using1112_stepwise_ruralnn_rainspline", replace
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae14_completedistrictsubsample_newvars_withselection_nonnormal", replace 
	restore 
...
addnotes("Sources: India National Sample Survey Office (NSSO) Surveys." "Using 2014, 2014 and 2011 Surveys to project into 2014." "a. Simulated" "b. Actual") ///
...
mat x=r(table)
matrix n=x[1,1..3]*(1-.316314)+x[1,4..6]*(.316314)
matrix n=n\x[1,4..6]
matrix n=n\x[1,1..3]
matrix n=n*100 

matrix n2=(p14[1,1]*(.316314)+p14[2,1]*(1-.316314))\p14[1..2,1]
matrix  n=n,n2
matrix list n 
matrix drop n2 
...
** 3.20 line 
matrix n_320=x[1,7..9]*(1-.316314)+x[1,10..12]*(.316314)
matrix n_320=n_320\x[1,10..12]
matrix n_320=n_320\x[1,7..9]
matrix n_320=n_320*100 

matrix n2=(p14_320[1,1]*(.316314)+p14_320[2,1]*(1-.316314))\p14_320[1..2,1]
...
** 5.50 line 
matrix n_550=x[1,13..15]*(1-.316314)+x[1,14..16]*(.316314)
matrix n_550=n_550\x[1,14..16]
matrix n_550=n_550\x[1,13..15]
matrix n_550=n_550*100 

matrix n2=(p14_550[1,1]*(.316314)+p14_550[2,1]*(1-.316314))\p14_550[1..2,1]
matrix  n_550=n_550,n2
matrix list n_550 
matrix drop n2 


/* OLD CODE UNUSED Using 2014
use  "$data\finaldata\hh_reg.dta", clear
keep if year==2014.5
...
mat n[2,1]=(x14[2,1])*100
mat n[3,1]=x14[1,1]*100
mat n[1,1] = (x14[2,1]*.316314 + x14[1,1]*(1-.316314))*100	

mat n[2,2]=(x14[2,1])*100
mat n[3,2]=(x14[1,1])*100
mat n[1,2]=(x14[2,1]*.316314 + x14[1,1]*(1-.316314))*100	

mat n[2,3]=(k14[1,4])
mat n[3,3]=(k14[2,4])
mat n[1,3]=(k14[1,4]*.316314 + k14[2,4]*(1-.316314))

mat n[2,4]=(k14[1,1])
mat n[3,4]=(k14[2,1])
mat n[1,4]=(k14[1,1]*.316314 + k14[2,1]*(1-.316314))
*/ 
...
posthead(& \multicolumn{1}{c}{\textbf{2014.5}} & {\textbf{2014.5}} & {\textbf{2011.5}} & {\color{red}\textbf{2014.5}}\\ "\hline") replace
...
replace year=2014.5 if year==1
replace year=2014.5 if year==2
...
replace labpos=5 if year==2014.5 
...
twoway  (line pov_320National year, lpattern(solid) lcolor(black) lwidth(thick)) (line pov_320Urban year, lpattern(dash) lcolor(purple) lwidth(thick)) (line pov_320Rural year, lpattern(longdash) lcolor(forest_green) lwidth(thick)) ///
		...
		label(3 "Rural") label(4 "Year of Survey")) xmticks(##2) xlabel(2014.5 (5) 2014.5) yscale(r(35 85))  ylabel(30(10)90, format(%2.0f)) ytitle(Percent) graphregion(color(white)) ///
...
twoway  (line pov_550National year, lpattern(solid) lcolor(black) lwidth(thick)) (line pov_550Urban year, lpattern(dash) lcolor(purple) lwidth(thick)) (line pov_550Rural year, lpattern(longdash) lcolor(forest_green) lwidth(thick)) ///
		...
		label(3 "Rural") label(4 "Year of Survey")) xmticks(##2) xlabel(2014.5 (5) 2014.5) yscale(r(70 100)) ylabel(70(5)100, format(%2.0f)) ytitle(Percent) graphregion(color(white)) ///
		note("Sources: India National Sample Survey (NSSO) Surveys.")
