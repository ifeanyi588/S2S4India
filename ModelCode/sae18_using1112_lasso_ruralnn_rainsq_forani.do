...
cap log using sae18_log_using1112_lasso_ruralnn_rainsq_forani, text replace
...
local dataout "$analysis\SAE_MATA18_using1112_lasso_ruralnn_rainsq_forani"
local dataoutu "$analysis\SAE_MATA18u_using1112_lasso_ruralnn_rainsq_forani"
local dataoutr "$analysis\SAE_MATA18r_using1112_lasso_ruralnn_rainsq_forani"


local ydump "$analysis\mata18_using1112_lasso_ruralnn_rainsq_forani"
local ydumpu "$analysis\mata18u_using1112_lasso_ruralnn_rainsq_forani"
local ydumpr "$analysis\mata18r_using1112_lasso_ruralnn_rainsq_forani"
...
*estimates save "$estimates/stage1u_18_using1112_lasso_ruralnn_rainsq_forani", replace 
...
*estimates save "$estimates/stage1r_18_using1112_lasso_ruralnn_rainsq_forani", replace 

qui: lassoregress _res_alfa $ruralvars [aw=pop_wgt]
local error_lasso_modelr "`e(varlist_nonzero)'"

************************************************* Second Stage - Urban **********************************************
sae data import, varlist(id_sae qhweight qhclust $urbanvars district_code hhid) uniqid(id_sae) area(district_code) ///
...
pline(57.791668) addvars(id_sae qhweight qhclust $urbanvars hhid) 
*/ 

* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_18_using1112_lasso_ruralnn_rainsq_forani)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_18_using1112_lasso_ruralnn_rainsq_forani.dta", replace

************************************************* Second Stage - Rural **********************************************
sae data import, varlist(id_sae qhweight qhclust $ruralvars district_code hhid) uniqid(id_sae) area(district_code) ///
...
pline(57.791668) addvars(id_sae qhweight qhclust $ruralvars district_code hhid) 
*/ 
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_18_using1112_lasso_ruralnn_rainsq_forani)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_18_using1112_lasso_ruralnn_rainsq_forani.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_18_using1112_lasso_ruralnn_rainsq_forani.dta", clear
	gen urban=1
	append using "$analysis\final_datar_18_using1112_lasso_ruralnn_rainsq_forani.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_18_using1112_lasso_ruralnn_rainsq_forani.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_18_using1112_lasso_ruralnn_rainsq_forani", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_18_using1112_lasso_ruralnn_rainsq_forani", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_using1112_lasso_ruralnn_rainsq_forani", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_using1112_lasso_ruralnn_rainsq_forani", replace
