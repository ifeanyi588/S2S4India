...
local dataout "$analysis\SAE_MATA_wd18_newvars"
local dataoutu "$analysis\SAE_MATAu_wd18_newvars"
local dataoutr "$analysis\SAE_MATAr_wd18_newvars"


local ydump "$analysis\mata_wd18_newvars"
local ydumpu "$analysis\matau_wd18_newvars"
local ydumpr "$analysis\matar_wd18_newvars"
...
local xvar111t `xvar111t' Dnoeduc Dprimary Dsecondary Dnoeduch Dprimaryh Dsecondaryh Dmarriedh Dmarried 
local xvar111t `xvar111t' Ddivorcedh Ddivorced Dnevermarriedh Dnevermarried Dmaleh Dmale Dfirewoodh Dfirewoodh 
...
local xvar011t `xvar011t' Dnoeduc Dprimary Dsecondary Dnoeduch Dprimaryh Dsecondaryh Dmarriedh Dmarried 
local xvar011t `xvar011t' Ddivorcedh Ddivorced Dnevermarriedh Dnevermarried Dmaleh Dmale Dfirewoodh Dfirewoodh 
...
estimates save "$estimates/stage1_18_indu_newvars", replace 
...
estimates save "$estimates/stage1_18_indr_newvars", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd18_newvars)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd18_newvars.dta", replace
...
uniqid(id_sae) vce(ell) rep(5) seed(16757) lny aggids(7) col(1) stage(second) ydump(`ydumpr') ///
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd18_newvars)
...
**** Predicted poverty
	...
	* Importing dataset (National)
	use "$analysis\final_datau_wd18_newvars.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd18_newvars.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd18_newvars.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd18_newvars", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd18_newvars", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae18_newvars", replace 
	
	** replicate subsample of compete distrits for comparison with 
	preserve 
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae18_completedistrictsubsample_newvars", replace 
	restore 
