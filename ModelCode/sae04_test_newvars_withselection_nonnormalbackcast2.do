...
cap log using sae04_log_test_newvars_withselection_nonnormal, text replace
...
local dataout "$analysis\SAE_MATA_wd04_newvars_withselection_nonnormal"
local dataoutu "$analysis\SAE_MATAu_wd04_newvars_withselection_nonnormal"
local dataoutr "$analysis\SAE_MATAr_wd04_newvars_withselection_nonnormal"


local ydump "$analysis\mata_wd04_newvars_withselection_nonnormal"
local ydumpu "$analysis\matau_wd04_newvars_withselection_nonnormal"
local ydumpr "$analysis\matar_wd04_newvars_withselection_nonnormal"
...
save "$data2\sae0911.dta", replace

* Urban
use "$data2\sae0911.dta", clear
keep if urban==1
...
estimates save "$estimates/stage1_04_indu_newvars_withselection_nonnormal", replace 
...
estimates save "$estimates/stage1_04_indr_newvars_withselection_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd04_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd04_newvars_withselection_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd04_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd04_newvars_withselection_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd04_newvars_withselection_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd04_newvars_withselection_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd04_newvars_withselection_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd04_newvars_withselection_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd04_newvars_withselection_nonnormal", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae04_newvars_withselection_nonnormal", replace 
