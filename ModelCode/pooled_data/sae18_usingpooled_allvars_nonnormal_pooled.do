...
cap log using sae18_log_test_newvars_pooled, text replace
...
local dataout "$analysis\SAE_MATA_usingall_allvars_nonnormal"
local dataoutu "$analysis\SAE_MATAu_usingall_allvars_nonnormal"
local dataoutr "$analysis\SAE_MATAr_usingall_allvars_nonnormal"


local ydump "$analysis\mata_usingall_allvars_nonnormal"
local ydumpu "$analysis\matau_usingall_allvars_nonnormal"
local ydumpr "$analysis\matar_usingall_allvars_nonnormal"
...
estimates save "$estimates/stage1_18_indu_usingall_allvars_nonnormal", replace 
...
estimates save "$estimates/stage1_18_indr_usingall_allvars_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_data18u_usingall_allvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu18_usingall_allvars_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_data18r_usingall_allvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr18_usingall_allvars_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_data18u_usingall_allvars_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_data18r_usingall_allvars_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_data18ur_usingall_allvars_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data18_usingall_allvars_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data18_usingall_allvars_nonnormal", clear
...
*/ 
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_usingall_allvars_nonnormal", replace 
	
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_usingall_allvars_nonnormal", replace
