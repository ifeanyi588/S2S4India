...
cap log using sae04_log_using0911_lasso_ruralnn_rainspline, text replace


* locals for directories and variables

local datain "$data2\sae04.dta"
local datainu "$data2\sae04uv2.dta"
local datainr "$data2\sae04rv2.dta"

local dataout "$analysis\SAE_MATA04_using0911_lasso_ruralnn_rainspline"
local dataoutu "$analysis\SAE_MATA04u_using0911_lasso_ruralnn_rainspline"
local dataoutr "$analysis\SAE_MATA04r_using0911_lasso_ruralnn_rainspline"


local ydump "$analysis\mata04_using0911_lasso_ruralnn_rainspline"
local ydumpu "$analysis\mata04u_using0911_lasso_ruralnn_rainspline"
local ydumpr "$analysis\mata04r_using0911_lasso_ruralnn_rainspline"
...
use "$data2/sae04u", replace 
foreach var of varlist $urbanvars {
cap gen `var't=`var'*t	
}
save "$data2/sae04uv2", replace 


use "$data2/sae04r", replace 
foreach var of varlist $urbanvars {
cap gen `var't=`var'*t	
}
save "$data2/sae04rv2", replace 
...
* keep if year == 2011.5 
keep if year==2009.5 | year==2011.5 

foreach var of varlist $urbanvars {
	cap gen `var't=`var'*t
	global urbanvars "$urbanvars `var't"
}

foreach var of varlist $ruralvars {
	cap gen `var't=`var'*t
	global ruralvars "$ruralvars `var't"
}
global urbanvars "$urbanvars t"
global ruralvars "$ruralvars t"
...
lasso linear lnwelf $urbanvars [iw=pop_wgt] if urban == 1, select(plugin, het)
glo urbanvars2 "`e(allvars_sel)'"

*qui: lassoregress lnwelf $ruralvars [aw=pop_wgt] if urban == 0
*glo ruralvars2 "`e(varlist_nonzero)'"

lasso linear lnwelf $ruralvars [iw=pop_wgt] if urban == 0, select(plugin, het)
glo ruralvars2 "`e(allvars_sel)'"
...
estimates save "$estimates/stage1u_04_using0911_lasso_ruralnn_rainspline", replace 

qui: lasso linear _res_alfa $ruralvars2 [iw=pop_wgt], select(plugin, het)
...
use "$data2\sae12.dta", clear
keep if urban==0
regress lnwelf $ruralvars2 [aw=pop_wgt], vce(robust) 

sae model povmap lnwelf $ruralvars2 [pw=pop_wgt], area(district_code) varest(h3) alfatest(_res) zvar($ruralvars2)
* sae model povmap lnwelf $ruralvars2 [pw=pop_wgt], area(district_code) varest(h3) alfatest(_res) 
estimates store indr

estimates save "$estimates/stage1r_04_using0911_lasso_ruralnn_rainspline", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_04_using0911_lasso_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_04_using0911_lasso_ruralnn_rainspline.dta", replace
*/
************************************************* Second Stage - Rural **********************************************
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_04_using0911_lasso_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_04_using0911_lasso_ruralnn_rainspline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_04_using0911_lasso_ruralnn_rainspline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_04_using0911_lasso_ruralnn_rainspline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_04_using0911_lasso_ruralnn_rainspline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_04_using0911_lasso_ruralnn_rainspline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_04_using0911_lasso_ruralnn_rainspline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae04_sectorpov_using0911_lasso_ruralnn_rainspline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae04_overallpov_using0911_lasso_ruralnn_rainspline", replace
