...
cap log using sae18_log_usingall_lasso_nonnormal_rainspline, text replace
set seed 4
...
local dataout "$analysis\SAE_MATA18_usingall_lasso_nonnormal_rainspline"
local dataoutu "$analysis\SAE_MATA18u_usingall_lasso_nonnormal_rainspline"
local dataoutr "$analysis\SAE_MATA18r_usingall_lasso_nonnormal_rainspline"


local ydump "$analysis\mata18_usingall_lasso_nonnormal_rainspline"
local ydumpu "$analysis\mata18u_usingall_lasso_nonnormal_rainspline"
local ydumpr "$analysis\mata18r_usingall_lasso_nonnormal_rainspline"
...
estimates save "$estimates/stage1u_18_usingall_lasso_nonnormal_rainspline", replace 
...
estimates save "$estimates/stage1r_18_usingall_lasso_nonnormal_rainspline", replace 

qui: lassoregress _res_alfa $ruralvars2 [aw=pop_wgt]
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_18_usingall_lasso_nonnormal_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_18_usingall_lasso_nonnormal_rainspline.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_18_usingall_lasso_nonnormal_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_18_usingall_lasso_nonnormal_rainspline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_18_usingall_lasso_nonnormal_rainspline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_18_usingall_lasso_nonnormal_rainspline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_18_usingall_lasso_nonnormal_rainspline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_18_usingall_lasso_nonnormal_rainspline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_18_usingall_lasso_nonnormal_rainspline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_usingall_lasso_nonnormal_rainspline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_usingall_lasso_nonnormal_rainspline", replace
