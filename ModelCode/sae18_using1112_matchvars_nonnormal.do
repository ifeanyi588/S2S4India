...
cap log using sae18_using1112_matchvars_nonnormal, text replace
...
local dataout "$analysis\SAE_MATA18_using1112_matchvars_nonnormal"
local dataoutu "$analysis\SAE_MATA18u_using1112_matchvars_nonnormal"
local dataoutr "$analysis\SAE_MATA18r_using1112_matchvars_nonnormal"


local ydump "$analysis\mata18_using1112_matchvars_nonnormal"
local ydumpu "$analysis\mata18u_using1112_matchvars_nonnormal"
local ydumpr "$analysis\mata18r_using1112_matchvars_nonnormal"
...
estimates save "$estimates/stage1_18_using1112_matchvars_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_data18u_using1112_matchvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu18_using1112_matchvars_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_data18r_using1112_matchvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr18_using1112_matchvars_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_data18u_using1112_matchvars_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_data18r_using1112_matchvars_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_data18ur_using1112_matchvars_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data18_using1112_matchvars_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data18_using1112_matchvars_nonnormal", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae18_sectorpov_using1112_matchvars_nonnormal", replace 
	
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae18_overallpov_using1112_matchvars_nonnormal", replace
