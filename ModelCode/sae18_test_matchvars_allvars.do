...
cap log using sae18_log_test_matchvars_allvars, text replace
...
local dataout "$analysis\SAE_MATA_wd18_allvars"
local dataoutu "$analysis\SAE_MATAu_wd18_allvars"
local dataoutr "$analysis\SAE_MATAr_wd18_allvars"


local ydump "$analysis\mata_wd18_allvars"
local ydumpu "$analysis\matau_wd18_allvars"
local ydumpr "$analysis\matar_wd18_allvars"
...
estimates save "$estimates/stage1_18_indu_wd_allvars", replace 
...
estimates save "$estimates/stage1_18_indr_wd_allvars", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd18_matchvars_allvars)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd18_allvars.dta", replace
...
uniqid(id_sae) vce(ell) rep(10) seed(16757) bootstrap lny aggids(7) col(1) stage(second) ydump(`ydumpr') ///
...
uniqid(id_sae) vce(ell) rep(10) seed(16757) lny aggids(7) col(1) stage(second) ydump(`ydumpr') ///
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd18_matchvars_allvars)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd18_matchvars_allvars.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd18_allvars.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd18_allvars.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd18_matchvars_allvars.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd18_matchvars_allvars", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd18_matchvars_allvars", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae18_wd_matchvars_allvars", replace 
	
	***estimate overall poverty
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae18_completedistrictsubsample_wd_matchvars_allvars", replace 
	restore 
