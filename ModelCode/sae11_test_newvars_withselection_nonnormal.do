...
cap log using sae11_log_test_newvars_withselection_nonnormal, text replace


* locals for directories and variables

local datain "$data2\sae11.dta"
local datainu "$data2\sae11u.dta"
local datainr "$data2\sae11r.dta"

local dataout "$analysis\SAE_MATA_wd11_newvars_withselection_nonnormal"
local dataoutu "$analysis\SAE_MATAu_wd11_newvars_withselection_nonnormal"
local dataoutr "$analysis\SAE_MATAr_wd11_newvars_withselection_nonnormal"


local ydump "$analysis\mata_wd11_newvars_withselection_nonnormal"
local ydumpu "$analysis\matau_wd11_newvars_withselection_nonnormal"
local ydumpr "$analysis\matar_wd11_newvars_withselection_nonnormal"
...
global ruralvars "Dagecat1h Dagecat2 Dagecat2h Dagecat3 Dagecat4h Dagecat5 Dagecat5h Dhhsizecat2h Dhhsizecat3" 
...
global ruralvars "${ruralvars} regwageurban regwageurbanh depratio depratioh Dhhsizecat3h hhage"
global ruralvars "${ruralvars} Dnoeduc Dprimary Dsecondary Dnoeduch Dprimaryh Dsecondaryh Dmarriedh Dmarried" 
global ruralvars "${ruralvars} Ddivorcedh Ddivorced Dnevermarriedh Dnevermarried Dmaleh Dmale" 
...
global urbanvars "${urbanvars} Dnoeduc Dprimary Dsecondary Dnoeduch Dprimaryh Dsecondaryh Dmarriedh Dmarried" 
global urbanvars "${urbanvars} Ddivorcedh Ddivorced Dnevermarriedh Dnevermarried Dmaleh Dmale hhage" 
...
*
keep if year == 2011.5
save "$data2\sae11.dta", replace

keep if urban == 1
save "$data2\sae11u.dta", replace

use "$data2\sae11.dta", clear
keep if urban == 0
save "$data2\sae11r.dta", replace
...
sae model povmap lnwelf $urbanvars2 [pw=pop_wgt], area(district_code) varest(ell) vce(ell) alfatest(robust) zvar($urbanvars2)
estimates table 

estimates store indu
estimates save "$estimates/stage1_11_indu_newvars_withselection_nonnormal", replace 
...
use "$data2\sae0410.dta", clear
keep if urban==0
sae model povmap lnwelf $ruralvars2 [pw=pop_wgt], area(district_code) varest(ell) vce(ell) alfatest(robust) zvar($ruralvars2)
estimates store indr

estimates save "$estimates/stage1_11_indr_newvars_withselection_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd11_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd11_newvars_withselection_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd11_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd11_newvars_withselection_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd11_newvars_withselection_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd11_newvars_withselection_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd11_newvars_withselection_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd11_newvars_withselection_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd11_newvars_withselection_nonnormal", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae11_newvars_withselection_nonnormal", replace 
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae11_completedistrictsubsample_newvars_withselection_nonnormal", replace 
	restore 
