...
cap log using sae18_log_test_matchvars_nonnormal, text replace
...
local dataout "$analysis\SAE_MATA_wd18_matchvars_nonnormal"
local dataoutu "$analysis\SAE_MATAu_wd18_matchvars_nonnormal"
local dataoutr "$analysis\SAE_MATAr_wd18_matchvars_nonnormal"


local ydump "$analysis\mata_wd18_matchvars_nonnormal"
local ydumpu "$analysis\matau_wd18_matchvars_nonnormal"
local ydumpr "$analysis\matar_wd18_matchvars_nonnormal"
...
estimates save "$estimates/stage1_18_indu_wd_matchvars_nonnormal", replace 
...
estimates save "$estimates/stage1_18_indr_wd_matchvars_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd18_matchvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd18_matchvars_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd18_matchvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd18_matchvars_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd18_matchvars_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd18_matchvars_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd18_matchvars_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd18_matchvars_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd18_matchvars_nonnormal", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae18_wd_matchvars_nonnormal", replace 
