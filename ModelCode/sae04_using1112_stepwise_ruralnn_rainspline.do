...
*****************************************************************************************************
*                                                                                                   *                          
***** --- ELL ESTIMATION: Uses the 2004.5, 2004.5 and 2011.5 surveys to impute into 2014-15 --- *****
*                                                                                                   *
*****************************************************************************************************/
...
cap log using sae04_log_using1112_stepwise_ruralnn_rainspline, text replace
...
local dataout "$analysis\SAE_MATA04_using1112_stepwise_ruralnn_rainspline"
local dataoutu "$analysis\SAE_MATA04u_using1112_stepwise_ruralnn_rainspline"
local dataoutr "$analysis\SAE_MATA04r_using1112_stepwise_ruralnn_rainspline"


local ydump "$analysis\mata04_using1112_stepwise_ruralnn_rainspline"
local ydumpu "$analysis\mata04u_using1112_stepwise_ruralnn_rainspline"
local ydumpr "$analysis\mata04r_using1112_stepwise_ruralnn_rainspline"
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_04_using1112_stepwise_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_04_using1112_stepwise_ruralnn_rainspline.dta", replace
...
estimates save "$estimates/stage1r_04_using1112_stepwise_ruralnn_rainspline", replace 
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_04_using1112_stepwise_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_04_using1112_stepwise_ruralnn_rainspline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_04_using1112_stepwise_ruralnn_rainspline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_04_using1112_stepwise_ruralnn_rainspline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_04_using1112_stepwise_ruralnn_rainspline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_04_using1112_stepwise_ruralnn_rainspline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_04_using1112_stepwise_ruralnn_rainspline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae04_sectorpov_using1112_stepwise_ruralnn_rainspline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae04_overallpov_using1112_stepwise_ruralnn_rainspline", replace
...
addnotes("Sources: India National Sample Survey Office (NSSO) Surveys." "Using 2004, 2004 and 2011 Surveys to project into 2014." "a. Simulated" "b. Actual") ///
...
matrix n2=(p14[1,1]*(.316304)+p14[2,1]*(1-.316304))\p14[1..2,1]
matrix  n=n,n2
matrix list n 
matrix drop n2 
...
matrix n2=(p14_320[1,1]*(.316304)+p14_320[2,1]*(1-.316304))\p14_320[1..2,1]
...
** 5.50 line 
matrix n_550=x[1,13..15]*(1-.316304)+x[1,14..16]*(.316304)
matrix n_550=n_550\x[1,14..16]
matrix n_550=n_550\x[1,13..15]
matrix n_550=n_550*100 

matrix n2=(p14_550[1,1]*(.316304)+p14_550[2,1]*(1-.316304))\p14_550[1..2,1]
...
mat n[2,3]=(k14[1,4])
mat n[3,3]=(k14[2,4])
mat n[1,3]=(k14[1,4]*.316304 + k14[2,4]*(1-.316304))

mat n[2,4]=(k14[1,1])
mat n[3,4]=(k14[2,1])
mat n[1,4]=(k14[1,1]*.316304 + k14[2,1]*(1-.316304))
*/ 
...
posthead(& \multicolumn{1}{c}{\textbf{2004.5}} & {\textbf{2004.5}} & {\textbf{2011.5}} & {\color{red}\textbf{2014.5}}\\ "\hline") replace
