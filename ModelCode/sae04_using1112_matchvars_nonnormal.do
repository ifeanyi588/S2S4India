...
cap log using sae04_using1112_matchvars_nonnormal, text replace
...
local dataout "$analysis\SAE_MATA04_using1112_matchvars_nonnormal"
local dataoutu "$analysis\SAE_MATA04u_using1112_matchvars_nonnormal"
local dataoutr "$analysis\SAE_MATA04r_using1112_matchvars_nonnormal"


local ydump "$analysis\mata04_using1112_matchvars_nonnormal"
local ydumpu "$analysis\mata04u_using1112_matchvars_nonnormal"
local ydumpr "$analysis\mata04r_using1112_matchvars_nonnormal"
...
estimates save "$estimates/stage1_04_using1112_matchvars_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_data04u_using1112_matchvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu04_using1112_matchvars_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_data04r_using1112_matchvars_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr04_using1112_matchvars_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_data04u_using1112_matchvars_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_data04r_using1112_matchvars_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_data04ur_using1112_matchvars_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data04_using1112_matchvars_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data04_using1112_matchvars_nonnormal", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae04_sectorpov_using1112_matchvars_nonnormal", replace 
	
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae04_overallpov_using1112_matchvars_nonnormal", replace
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae18_completedistrictsubsample_wd_matchvars_nonnormal", replace 
	restore 
