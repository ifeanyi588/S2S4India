...
*****************************************************************************************************
*                                                                                                   *                          
***** --- ELL ESTIMATION: Uses the 2009.5, 2009.5 and 2011.5 surveys to impute into 2014-15 --- *****
*                                                                                                   *
*****************************************************************************************************/
...
cap log using sae09_log_using1112_stepwise_ruralnn_rainspline, text replace
...
local dataout "$analysis\SAE_MATA09_using1112_stepwise_ruralnn_rainspline"
local dataoutu "$analysis\SAE_MATA09u_using1112_stepwise_ruralnn_rainspline"
local dataoutr "$analysis\SAE_MATA09r_using1112_stepwise_ruralnn_rainspline"


local ydump "$analysis\mata09_using1112_stepwise_ruralnn_rainspline"
local ydumpu "$analysis\mata09u_using1112_stepwise_ruralnn_rainspline"
local ydumpr "$analysis\mata09r_using1112_stepwise_ruralnn_rainspline"
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_09_using1112_stepwise_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_09_using1112_stepwise_ruralnn_rainspline.dta", replace
...
estimates save "$estimates/stage1r_09_using1112_stepwise_ruralnn_rainspline", replace 
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_09_using1112_stepwise_ruralnn_rainspline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_09_using1112_stepwise_ruralnn_rainspline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_09_using1112_stepwise_ruralnn_rainspline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_09_using1112_stepwise_ruralnn_rainspline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_09_using1112_stepwise_ruralnn_rainspline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_09_using1112_stepwise_ruralnn_rainspline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_09_using1112_stepwise_ruralnn_rainspline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae09_sectorpov_using1112_stepwise_ruralnn_rainspline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae09_overallpov_using1112_stepwise_ruralnn_rainspline", replace
...
addnotes("Sources: India National Sample Survey Office (NSSO) Surveys." "Using 2009, 2009 and 2011 Surveys to project into 2014." "a. Simulated" "b. Actual") ///
...
posthead(& \multicolumn{1}{c}{\textbf{2009.5}} & {\textbf{2009.5}} & {\textbf{2011.5}} & {\color{red}\textbf{2014.5}}\\ "\hline") replace
...
twoway  (line pov_320National year, lpattern(solid) lcolor(black) lwidth(thick)) (line pov_320Urban year, lpattern(dash) lcolor(purple) lwidth(thick)) (line pov_320Rural year, lpattern(longdash) lcolor(forest_green) lwidth(thick)) ///
		...
		label(3 "Rural") label(4 "Year of Survey")) xmticks(##2) xlabel(2009.5 (5) 2014.5) yscale(r(35 85))  ylabel(30(10)90, format(%2.0f)) ytitle(Percent) graphregion(color(white)) ///
...
twoway  (line pov_550National year, lpattern(solid) lcolor(black) lwidth(thick)) (line pov_550Urban year, lpattern(dash) lcolor(purple) lwidth(thick)) (line pov_550Rural year, lpattern(longdash) lcolor(forest_green) lwidth(thick)) ///
		...
		label(3 "Rural") label(4 "Year of Survey")) xmticks(##2) xlabel(2009.5 (5) 2014.5) yscale(r(70 100)) ylabel(70(5)100, format(%2.0f)) ytitle(Percent) graphregion(color(white)) ///
		note("Sources: India National Sample Survey (NSSO) Surveys.")
