...
cap log close
*cap log using sae10_log_test_matchvars, text replace 
*cap log using sae10_log_test_matchvars_notime, text replace
*cap log using sae10_log_test_newvars, text replace
*cap log using sae10_log_test_newvars_withselection, text replace
cap log using sae10_log_using1112_wsel_nonnormal, text replace
...
local dataout "$analysis\SAE_MATA10_using1112_wsel_nonnormal"
local dataoutu "$analysis\SAE_MATA10u_using1112_wsel_nonnormal"
local dataoutr "$analysis\SAE_MATA10r_using1112_wsel_nonnormal"


local ydump "$analysis\mata10_using1112_wsel_nonnormal"
local ydumpu "$analysis\mata10u_using1112_wsel_nonnormal"
local ydumpr "$analysis\mata10r_using1112_wsel_nonnormal"
...
estimates save "$estimates/stage1u_10_using1112_wsel_nonnormal", replace 
...
estimates save "$estimates/stage1r_10_using1112_wsel_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_10_using1112_wsel_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_10_using1112_wsel_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_10_using1112_wsel_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_10_using1112_wsel_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_10_using1112_wsel_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_datar_10_using1112_wsel_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_10_using1112_wsel_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_10_using1112_wsel_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_10_using1112_wsel_nonnormal", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae10_sectorpov_using1112_wsel_nonnormal", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae10_overallpov_using1112_wsel_nonnormal", replace
