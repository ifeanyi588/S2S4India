...
cap log using sae09_log_using1112_matchvars_nonnormal2_spline, text replace
...
local dataout "$analysis\SAE_MATA09_using1112_matchvars_nonnormal2_spline"
local dataoutu "$analysis\SAE_MATA09u_using1112_matchvars_nonnormal2_spline"
local dataoutr "$analysis\SAE_MATA09r_using1112_matchvars_nonnormal2_spline"


local ydump "$analysis\mata09_using1112_matchvars_nonnormal2_spline"
local ydumpu "$analysis\mata09u_using1112_matchvars_nonnormal2_spline"
local ydumpr "$analysis\mata09r_using1112_matchvars_nonnormal2_spline"
...
global ruralvars "Dagecat1h Dagecat2h Dagecat3h Dagecat4h Dagecat5h Dagecat1 Dagecat2 Dagecat3 Dagecat4 Dagecat5" 
...
global urbanvars "Dagecat1h Dagecat2h Dagecat3h Dagecat4h Dagecat5h Dagecat1 Dagecat2 Dagecat3 Dagecat4 Dagecat5" 
...
global urbanvars "${urbanvars} regwageurban regwageurbanh"
...
estimates save "$estimates/stage1u_09_using1112_matchvars_nonnormal2_spline", replace 
...
estimates save "$estimates/stage1r_09_using1112_matchvars_nonnormal2_spline", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_09_using1112_matchvars_nonnormal2_spline)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_09_using1112_matchvars_nonnormal2_spline.dta", replace
...
keep if urban==0
/* assume normality  
sae sim povmap lnwelf $ruralvars2 [pw=pop_wgt], area(district_code) varest(h3) eta(nonnormal) epsilon(nonnormal) ///
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_09_using1112_matchvars_nonnormal2_spline)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_09_using1112_matchvars_nonnormal2_spline.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_09_using1112_matchvars_nonnormal2_spline.dta", clear
	gen urban=1
	append using "$analysis\final_datar_09_using1112_matchvars_nonnormal2_spline.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_09_using1112_matchvars_nonnormal2_spline.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_09_using1112_matchvars_nonnormal2_spline", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_09_using1112_matchvars_nonnormal2_spline", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae09_sectorpov_using1112_matchvars_nonnormal2_spline", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae09_overallpov_using1112_matchvars_nonnormal2_spline", replace
