...
cap log close
*cap log using sae12_log_test_matchvars, text replace 
*cap log using sae12_log_test_matchvars_notime, text replace
*cap log using sae12_log_test_newvars, text replace
*cap log using sae12_log_test_newvars_withselection, text replace
cap log using sae12_log_test_newvars_withselection_nonnormal3, text replace


* locals for directories and variables

local datain "$data2\sae12.dta"
local datainu "$data2\sae12u.dta"
local datainr "$data2\sae12r.dta"

local dataout "$analysis\SAE_MATA_wd12_newvars_withselection_nonnormal"
local dataoutu "$analysis\SAE_MATAu_wd12_newvars_withselection_nonnormal"
local dataoutr "$analysis\SAE_MATAr_wd12_newvars_withselection_nonnormal"


local ydump "$analysis\mata_wd12_newvars_withselection_nonnormal"
local ydumpu "$analysis\matau_wd12_newvars_withselection_nonnormal"
local ydumpr "$analysis\matar_wd12_newvars_withselection_nonnormal"
...
global ruralvars "Dagecat1h Dagecat2 Dagecat2h Dagecat5 Dagecat5h Dhhsizecat2h" 
global ruralvars "${ruralvars} Dhhsizecat4h Dhhsizecat5 Dhhsizecat5h highskillocc highskillocch" 
global ruralvars "${ruralvars} middleskillocc middleskillocch ind scst scsth selfemployed selfemployedh" 
global ruralvars "${ruralvars} depratio depratioh Dhhsizecat3h hhage"
global ruralvars "${ruralvars} Dnoeduc Dnoeduch Dmarriedh Dmarried" 
global ruralvars "${ruralvars} Dnevermarried Dmaleh Dmale" 
global ruralvars "${ruralvars} Ddungcakeh Ddungcake Dnocookingh Dcokecoalh Dcokecoal Dkeroseneh"
...
global urbanvars "Dagecat1h Dagecat2h Dagecat5 Dagecat5h Dhhsizecat2h" 
global urbanvars "${urbanvars} Dhhsizecat4 Dhhsizecat4h Dhhsizecat5 Dhhsizecat5h highskillocc highskillocch" 
global urbanvars "${urbanvars} middleskillocc middleskillocch ind indh scst scsth selfemployedh" 
global urbanvars "${urbanvars} regwageurban regwageurbanh depratio depratioh Dhhsizecat3h"
global urbanvars "${urbanvars} Dmarriedh Dmarried" 
global urbanvars "${urbanvars} Dnevermarriedh Dnevermarried Dmaleh Dmale hhage" 
global urbanvars "${urbanvars} Dfirewood Ddungcakeh Ddungcake Dnocookingh Dcokecoalh Dkeroseneh"
global urbanvars "${urbanvars} Delectricityh Dcharcoalh Dlpg Dlpgh"  
...
stepwise, pr(0.05): reg lnwelf $urbanvars [pw = pop_wgt] if urban == 1
local temp : colfullnames e(b) 
local b _cons
local xvars : list temp - b
glo urbanvars `xvars'

stepwise, pr(0.05): reg lnwelf $ruralvars [pw = pop_wgt] if urban == 0
local temp : colfullnames e(b) 
local b _cons
local xvars : list temp - b
glo ruralvars `xvars'
...
*summ $urbanvars if urban==1 & year==2014.5 [aw=pop_wgt]


**begin to create the datasets we need for 2014 imputations and redo some 
**of the above commands to create certain variables
/*
keep if year > 2014
save "$data2\sae14.dta", replace

keep if urban == 1
save "$data2\sae14u.dta", replace

use "$data2\sae14.dta", clear
keep if urban == 0
save "$data2\sae14r.dta", replace

use "$path\data\finaldata\hhmaster.dta", clear
drop Dagecat6 Dagecat6h Dhhsizecat6 Dhhsizecat6h
unab ruralvars : $ruralvars 
unab urbanvars : $urbanvars 

gen lnwelf = ln(welfareintr)
*local urbanvars : subinstr local urbanvars "Dhhsizecat4ht" "" 
*local urbanvars : subinstr local urbanvars "Dhhsizecat5ht" "" 
local urbanvars : subinstr local urbanvars "Dagecat4ht" "" 
local urbanvars : subinstr local urbanvars "casualurbanht" "" 
local urbanvars : subinstr local urbanvars "regwageurbanht" ""  
...
save "$data2\sae0910.dta", replace

* Urban
use "$data2\sae0910.dta", clear
keep if urban==1


regress lnwelf $urbanvars [aw=pop_wgt], vce(robust) 



sae model povmap lnwelf $urbanvars [pw=pop_wgt], area(district_code) varest(ell) vce(ell) alfatest(robust) zvar($urbanvars)
estimates table 

estimates store indu
estimates save "$estimates/stage1_12_indu_newvars_withselection_nonnormal", replace 
...
use "$data2\sae0910.dta", clear
keep if urban==0
sae model povmap lnwelf $ruralvars [pw=pop_wgt], area(district_code) varest(ell) vce(ell) alfatest(robust) zvar($ruralvars)
estimates store indr

estimates save "$estimates/stage1_12_indr_newvars_withselection_nonnormal", replace 
...
* non-paramertric
sae sim povmap lnwelf $urbanvars [pw=pop_wgt], area(district_code) varest(ell) eta(nonnormal) epsilon(nonnormal) ///
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd12_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd12_newvars_withselection_nonnormal.dta", replace
...
keep if urban==0
/* assume normality  
sae sim povmap lnwelf $ruralvars [pw=pop_wgt], area(district_code) varest(ell) eta(normal) epsilon(normal) ///
...
sae sim povmap lnwelf $ruralvars [pw=pop_wgt], area(district_code) varest(ell) eta(nonnormal) epsilon(nonnormal) ///
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd12_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd12_newvars_withselection_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd12_newvars_withselection_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd12_newvars_withselection_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd12_newvars_withselection_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd12_newvars_withselection_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd12_newvars_withselection_nonnormal", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae12_newvars_withselection_nonnormal", replace 
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae12_completedistrictsubsample_newvars_withselection_nonnormal", replace 
	restore 
