...
cap log close
*cap log using sae15_log_test_matchvars, text replace 
*cap log using sae15_log_test_matchvars_notime, text replace
*cap log using sae15_log_test_newvars, text replace
*cap log using sae15_log_test_newvars_withselection, text replace
cap log using sae15_log_test_newvars_withselection_nonnormal, text replace


* locals for directories and variables

local datain "$data2\sae15.dta"
local datainu "$data2\sae15u.dta"
local datainr "$data2\sae15r.dta"

local dataout "$analysis\SAE_MATA_wd15_newvars_withselection_nonnormal"
local dataoutu "$analysis\SAE_MATAu_wd15_newvars_withselection_nonnormal"
local dataoutr "$analysis\SAE_MATAr_wd15_newvars_withselection_nonnormal"


local ydump "$analysis\mata_wd15_newvars_withselection_nonnormal"
local ydumpu "$analysis\matau_wd15_newvars_withselection_nonnormal"
local ydumpr "$analysis\matar_wd15_newvars_withselection_nonnormal"
...
global ruralvars "${ruralvars} Dnevermarriedh Dnevermarried Dmaleh Dmale" 
...
estimates save "$estimates/stage1_15_indu_newvars_withselection_nonnormal", replace 
...
estimates save "$estimates/stage1_15_indr_newvars_withselection_nonnormal", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_wd15_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impu_wd15_newvars_withselection_nonnormal.dta", replace
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_wd15_newvars_withselection_nonnormal)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0) plines(57.791668) 

save "$res\impr_wd15_newvars_withselection_nonnormal.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_wd15_newvars_withselection_nonnormal.dta", clear
	gen urban=1
	append using "$analysis\final_datar_wd15_newvars_withselection_nonnormal.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_wd15_newvars_withselection_nonnormal.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_wd15_newvars_withselection_nonnormal", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_wd15_newvars_withselection_nonnormal", clear
...
*/ 
	...
	*/ 
	...
	estimates save "D:\Poverty_Estimates_India\estimates\replication\sae15_newvars_withselection_nonnormal", replace 
	...
	*keep if Nyearsdistrict==4 
	mi estimate: svy: mean Dpoor* povgap* povseverity*, over(urban)
	estimates save "D:\Poverty_Estimates_India\estimates\sae15_completedistrictsubsample_newvars_withselection_nonnormal", replace 
	restore 
