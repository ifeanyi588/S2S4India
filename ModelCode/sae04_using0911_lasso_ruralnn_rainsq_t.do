...
cap log using sae04_log_using0911_lasso_ruralnn_rainsq_t, text replace
...
local dataout "$analysis\SAE_MATA04_using0911_lasso_ruralnn_rainsq_t"
local dataoutu "$analysis\SAE_MATA04u_using0911_lasso_ruralnn_rainsq_t"
local dataoutr "$analysis\SAE_MATA04r_using0911_lasso_ruralnn_rainsq_t"


local ydump "$analysis\mata04_using0911_lasso_ruralnn_rainsq_t"
local ydumpu "$analysis\mata04u_using0911_lasso_ruralnn_rainsq_t"
local ydumpr "$analysis\mata04r_using0911_lasso_ruralnn_rainsq_t"
...
keep if year == 2011.5 | year == 2009.5
...
estimates save "$estimates/stage1u_04_using0911_lasso_ruralnn_rainsq_t", replace 
...
xx
estimates save "$estimates/stage1r_04_using0911_lasso_ruralnn_rainsq_t", replace 
...
* Export data
sae data export, matasource(`ydumpu') numfiles(1) datasave($analysis\final_datau_04_using0911_lasso_ruralnn_rainsq_t)

* Poverty numbers
sae proc indicator, matasource(`ydumpu') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impu_04_using0911_lasso_ruralnn_rainsq_t.dta", replace
...
uniqid(id_sae) vce(ell) rep(100) seed(57678) lny aggids(7) col(1) stage(second) ydump(`ydumpr') ///
...
* Export data
sae data export, matasource(`ydumpr') numfiles(1) datasave($analysis\final_datar_04_using0911_lasso_ruralnn_rainsq_t)

* Poverty numbers
sae proc indicator, matasource(`ydumpr') aggids(7 8) indicators(fgt0 gini) plines(57.791668) 

save "$res\impr_04_using0911_lasso_ruralnn_rainsq_t.dta", replace

*************************************************************************************************************************
**** Predicted poverty

	* Importing dataset (National)
	use "$analysis\final_datau_04_using0911_lasso_ruralnn_rainsq_t.dta", clear
	gen urban=1
	append using "$analysis\final_datar_04_using0911_lasso_ruralnn_rainsq_t.dta"
	replace urban=0 if urban==.
	save "$analysis\final_dataur_04_using0911_lasso_ruralnn_rainsq_t.dta", replace
	...
	* Importing dataset into MI format
	...
	save "$analysis\mi_data_04_using0911_lasso_ruralnn_rainsq_t", replace 
...
global max_welfareu=r(max) 
	...
	use "$analysis\mi_data_04_using0911_lasso_ruralnn_rainsq_t", clear
	...
	*/ 
	...
	estimates save "$pathout\estimates\sae04_sectorpov_using0911_lasso_ruralnn_rainsq_t", replace 
	
	**estimates for the overall national
	mi estimate: svy: mean Dpoor* povgap* povseverity*
	estimates save "$pathout\estimates\sae04_overallpov_using0911_lasso_ruralnn_rainsq_t", replace
